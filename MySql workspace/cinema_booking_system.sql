...
select bookings.customer_id,count(reserved_seat.seat_id) from bookings
join reserved_seat on bookings.id=reserved_seat.booking_id
join screenings on reserved_seat.id=screenings.id
group by bookings.customer_id
having screenings.start_time between '2017-10-01' and '2017-10-30';

select films.name,films.length_min,count(bookings.screening_id) from films
join bookings on films.id=bookings.id
group by films.name
